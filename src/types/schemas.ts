...
const KnittingMachineSchema = z.object({
  ...
  machineBrand: z.union([
    ...
    z.string().min(1, 'Machine brand is required'),
  ]),
...
export const DyeingMachineSchema = z.discriminatedUnion('DyeingMachineType', [
  z.object({
    ...
    typeOfFabric: z.enum(['Tubular', 'open width']).optional(),
...
export const YarnProcessingMachineSchema = z.object({
  typeOfYarnProcessingMachine: z.enum([
    'Yarn Dyeing',
    'Yarn Twisting',
    'Cone-Winding',
  ]),
...
const StitchingMachineSchema = z.object({
  machineType: z.union([
    ...
    z.string().min(1, 'Machine type is required'),
  ]),
  noOfMachines: z.number().int().positive().optional(),
});
...
const IronPackageMachineSchema = z.object({
  noOfTables: z
    .number()
    .int()
    .min(0, 'Number of tables cannot be negative')
    .optional(),
  noOfMachines: z
    .number()
    .int()
...
const KajaButtonMachineSchema = z.object({
  machineType: z.union([
    z.enum([' ⁠Kaj(a)', 'Button', 'Press button', 'others']),
    z.string().min(1, 'Machine type is required'),
  ]),
  noOfMachines: z
    .number()
...
export const onBoardCompany = z
  .object({
    ...
    productionCapacity: z.number().int().positive(),
...
export const createJobPostSchema = z.object({
  ...
  detailedDescription: z
    ...
    .optional(),
  jobImages: z.array(z.url()).optional().default([]),
